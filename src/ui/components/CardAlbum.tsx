...
export const CardAlbum = ({ artist, album }: CardAlbumProps) => {
  const getRandom = (min:number, max: number)=>(Math.random() * (max - min) + min).toLocaleString('pe', { maximumFractionDigits: 2 });
  return (
        ...
        <div className='flex-1 overflow-hidden p-8 flex flex-col gap-3 md:gap-5 text-white bg-center bg-no-repeat bg-cover bg-primary-variant bg-opacity-70 relative'>
            ...
            <div className="w-full flex flex-col">
              ...
              <div className="flex flex-wrap gap-x-4 w-full">
                ...
                <span className="text-secondary">{getRandom(4565656, 9999945)} seguidores</span>
              </div>
            </div>
            <p className="h-full w-full line-clamp-3">Adele Laurie Blue Adkins (Tottenham, Londres, Inglaterra, 5 de mayo de 1988), conocida simplemente como Adele, es una cantante, compositora y multinstrumentista británica.8​</p>
