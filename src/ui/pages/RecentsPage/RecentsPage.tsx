...
const List = ({ tracks, query }: {tracks: TrackMdl[], query: string }) => {
  ...
  if(tracks.length === 0){
    return (
      <section aria-label='Seach Result' className='flex-1 flex justify-center items-center'>
  ...
  return (
    <>
      ...
      <section aria-label="Song Results" className='flex flex-col gap-2 sm:gap-3 md:gap-4 lg:gap-5'>
        <h2 className='text-primary'>Resultados</h2>
        <ul role="list" aria-label="Song list" className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 3xl:grid-cols-7 gap-[22px]'>
          {
            tracks.map((track, i)=>(
...
const LoadSection = ()=> {
  ...
  if(loading)
  return (
    <section aria-label='Seach Error' className='flex-1 flex justify-center items-center'>
